...
// This function uses the classic ("V1") Netlify Functions format, which needs
// Blobs configured manually (unlike the newer V2 format, where it's automatic).
// NETLIFY_BLOBS_TOKEN is a Netlify personal access token, set as an env var.
function blobStore(name) {
  return getStore({
    name,
    siteID: "b6dcb1e8-f9be-45d5-be9c-36ad663a10d8",
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const licenses = blobStore("licenses");
    const customers = blobStore("customers");
